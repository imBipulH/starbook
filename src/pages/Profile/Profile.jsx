...
const Profile = () => {
  ...
  //     if (!editBoxRef.current.contains(event.target)) {
  //       setEdit(false)
  //     }
  //   }
  ...
  return (
    <>
      {data && (
        <div className='bg-primary h-screen w-full overflow-y-auto'>
          ...
          {edit && (
            <div
              ref={editBoxRef}
              className='absolute text-white p-4 z-10 top-1/2 left-1/2 translate-x-[-50%] translate-y-[-50%]  bg-gray-600 w-[800px] h-[400px] box-border'
            >
              <h3 className='py-2 font-sans font-bold text-lg'>About Me:</h3>
              <textarea
                name='Bio'
                value={newContent}
                onChange={e => setNewContent(e.target.value)}
                className='w-full h-[80%] box-border bg-gray-600 p-3 border'
              />
              <button
                onClick={handleSave}
                className='absolute right-1 bottom-1 text-white border border-gray-500 px-2 rounded-sm'
              >
                Save
              </button>
            </div>
              ...
              <div className='w-full mt-4  flex justify-start items-center gap-8 relative'>
                ...
                <div className='w-full grow '>
                  {!edit && (
                    <button
                      onClick={handleEdit}
                      className='edit-button absolute right-0 bottom-[-10px] text-white border border-gray-500 px-2 rounded-sm'
                    >
                      Edit
                    </button>
                  )}
